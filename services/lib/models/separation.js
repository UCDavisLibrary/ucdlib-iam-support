...
class UcdlibSeparation{
  ...
  async getEmployeeRecord(separationId){
    const out = {error: false, separationRecord: null, employeeRecord: null};
    let separationRecord = await this.getById(separationId);
    if ( !separationRecord.res?.rowCount ){
      out.error = true;
      out.message = `Separation request with id ${separationId} does not exist`;
      return out;
    }
    out.separationRecord = separationRecord.res.rows[0];
    const iamId = out.separationRecord.iam_id;
    if ( !iamId ) {
      out.error = true;
    ...
    out.employeeRecord = employeeRecord.res.rows[0];
  ...
  async query(q){
    ...
    if ( q.limit && !isNaN(parseInt(q.limit)) ) {
      text = `${text} LIMIT ${parseInt(q.limit)}`;
    }
