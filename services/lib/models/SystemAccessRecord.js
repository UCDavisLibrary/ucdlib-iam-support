...
class SystemAccessRecord {
  ...
  static separationRecordProp = 'removedFromSystems';
  get separationRecordProp(){
    return SystemAccessRecord.separationRecordProp;
  }
  ...
  async writeToSeparationRequest(separationRequestId){
    if ( !separationRequestId ) {
      throw new Error('SystemAccessRecord: separationRequestId is required to save access records');
    }
    ...
    if ( !existing.res.rowCount ) {
      throw new Error(`SystemAccessRecord: No separation request found with id ${separationRequestId}`);
    }
