...
class iamAdmin {
  ...
  async sendAdoptionRtNotification(rtTicketId){
    if ( !rtTicketId ) {
      return {error: true, message: 'No RT ticket id provided'};
    }
    ...
    if ( rtResponse.err )  {
      ...
      return {error: true, message: `Error sending RT notification for employee adoption`};
    }
    return {error: false, message: 'RT notification sent'};
  }

  async sendSeparationNotification(rtTicketId) {
    if ( !rtTicketId ) {
      return {error: true, message: 'No RT ticket id provided'};
    }
    if ( config.rt.forbidWrite ) {
    ...
    if ( rtResponse.err )  {
      ...
      return {error: true, message: `Error sending RT notification for employee separation`};
    }
    return {error: false, message: 'RT notification sent', rtSent: true};
  }
  ...
  async deprovisionKcAccount(userId, opts={}){
    ...
    if ( !userId ) {
      out.error = true;
      out.message = `${out.message} - No user id provided`;
      return out;
    }
    try {
      models.keycloakAdmin.resetState();
  ...
  async deleteEmployeeRecord(id, options={}){
    ...
    if ( !id ) {
      out.error = true;
      out.message = `${out.message} - No id provided`;
      return out;
    }
    ...
    if ( !iamId ) {
      out.error = true;
      out.message = `${out.message} - Employee ${idType}:${id} does not have an IAM id`;
      return out;
    }
    ...
    if ( isHeadOf.length && !options.force ) {
      out.error = 'isHeadOf';
      out.message = `${out.message} - Employee ${idType}:${id} is a head of one or more groups. Please remove head of groups first. You can use --force to override this check.`;
      out.isHeadOf = isHeadOf;
      return out;
    }
  ...
  /**
   * @description Check if an employee is a supervisor of employee in separation record based on IAM id and separation record d
   * @param {string} iamId - employee IAM id
  ...
  async sendSeparationReminder(record, params={}) {
    ...
    const dayAfter = new Date(separationDate);
    ...
    if ( !employee.res.rows.length ) {
      out.log.message = 'Employee not found, which likely means they have already been separated';
      return out;
    }
  ...
  async sendFacilitiesRtRequest(idOrRecord, params={}) {
    ...
    for (const p of permissionsFormProperties) {
      ...
      if ( p.payload === 'permissions.facilities.aggieAccess' ) continue;
