...
class KeycloakAdmin{
  ...
  constructor(){
    ...
    this.groupAttributes = [
      ...
      {kcName: 'type', entity: 'group', entityProp: 'type_name'},
      {kcName: 'nameShort', entity: 'group', entityProp: 'name_short'},
      {kcName: 'partOfOrg', entity: 'group', entityProp: 'part_of_org'},
    ]
    this.resetState();
  }
  ...
  getCachedKcGroup(id, idType='kcId'){
    if ( idType === 'kcId' ){
      ...
      return this.kcGroups.find(g => {
        id = id.toString();
        const attrs = g.attributes.dbId || [];
  ...
  async syncGroups(remove=false){
    ...
    console.log(this.getCachedKcGroup('8d017235-1c0f-4c09-a93f-1605ecb77ac5', 'kcId'));
    ...
    for ( const parentGroupId in groupsWithChildren ){
      ...
      for (const childId of children) {
        if ( !currentChildren.includes(childId) ){
          ...
          console.log(parentGroupId, child);
