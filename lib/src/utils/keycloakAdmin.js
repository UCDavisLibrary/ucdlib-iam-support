...
class KeycloakAdmin{
  ...
  async getAllGroups(){
    ...
    let groups = await this.getGroups({briefRepresentation: false, max: 1000, populateHierarchy: true});
    for (const group of groups) {
      await this.getSubGroups(group);
    }
  ...
  /**
   * @description Recursively retrieve all subgroups of a group
   * @param {Object} group
   * @returns {Object} - With a group.subGroups property added
   */
  async getSubGroups(group, args={}){
    if ( !Object.keys(args).length ){
      args = {briefRepresentation: false, max: 1000};
    }
    args.parentId = group.id;
    if ( !args.parentId || !group.subGroupCount ) {
      group.subGroups = [];
      return group;
    }
    ...
    let subGroups = await this.client.groups.listSubGroups(args);
    ...
    for ( const subGroup of subGroups ){
      await this.getSubGroups(subGroup, args);
    }
    return group;
  }
  ...
  async getSupervisorsGroup(){
    ...
    if ( supervisorsGroup ){
      if ( !supervisorsGroup.members.length && !this.retrievedSupervisorMembership ) {
        supervisorsGroup.members = await this.client.groups.listMembers({id: supervisorsGroup.id, briefRepresentation: true});
        this.retrievedSupervisorMembership = true;
      }
      return supervisorsGroup;
    }
  ...
  /**
   * @description Syncs the 'Supervisors' kc group with supervisors from the local database
   * @param {Boolean} remove - If true, will remove users from the group that are not supervisors
   */
  ...
  async syncGroups(remove=false){
    ...
    for ( const dbGroup of this.dbGroups ){
      ...
      if ( !kcGroup ){
        if ( !dbGroup.archived) {
          await this.createGroup(dbGroup, true);
        }
      } else {
        if ( dbGroup.archived ){
          if ( remove ){
   ...
   * @param {Boolean} refreshGroupCache - If true, will completely refresh the group cache after creating the group
   */
  async createGroup(dbGroup, refreshGroupCache=false){
    if ( !this.refreshInterval ){
      await this.refreshAccessToken();
    }
  ...
  async updateUser(userId, data, onlyUseCache=false){
    ...
    await this.client.users.update({id: user.id}, data);
    const updatedUser = await this.getUsers({username: userId});
    this.users[userId] = updatedUser[0];
  ...
  async deleteUser(userId, onlyUseCache=false){
    ...
    const r = await this.client.users.del({id: kcId});
    return r;
  }
  ...
  /**
   * @description Returns a summary of all logs
   */
  logSummary(){
    const summary = {};
