...
class UcdlibEmployees {
 ...
 /**
   ...
   * @returns
   */
  async updateMetadata(id, key, value, employeeId, idType="employeeId") {
    const toUpdate = {};
    ...
    toUpdate['metadata_key'] = key;
    toUpdate['metadata_value'] = value;
    toUpdate['employee_id'] = employeeId;
  ...
  async getById(id, idType='id', options={}){
    ...
    const text = `
      ...
      ` : ''}
      ${includeMetadata ? `
        LEFT JOIN metadata as md on e.employee_id = md.employee_id
      ` : ''}
      WHERE
