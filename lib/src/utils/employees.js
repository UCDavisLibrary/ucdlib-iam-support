...
class UcdlibEmployees {
  ...
  async dismissRecordDiscrepancyNotifications(iamId, ids=[]){
    ...
    if(ids.length) {
      params.push(...ids);
      where.push(`id IN ${pg.valuesArray(ids)}`);
    }

    params.push(String(iamId));         
    if(iamId) where.push(`iam_id = $${params.length}`);
  ...
  async getActiveRecordDiscrepancyNotifications(olderThanInterval, iamId = ''){

    const text = `
      SELECT *
      FROM outdated_records
      WHERE NOT fixed
      ${iamId !== '' ? `AND iam_id = '${iamId}'` : ''}
