...
export default class AccessToken {
  ...
  get canQueryUcdIam(){
    if ( this.hasBasicAccess ) return true;
    if ( this.canCreateRequests ) return true;
    return false;
  }

  get canDoPatronSearch(){
    if ( this.hasAdminAccess ) return true;
    if ( this.hasHrAccess ) return true;
    if ( this._inRoleList('search-patrons', 'resource') ) return true;
    return false;
  }
  ...
  get canUploadOrgChart(){
    return this._inRoleList('orgchart');
  }
