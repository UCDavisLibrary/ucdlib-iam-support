...
class iamAdmin {
  ...
  async adoptEmployee(onboardingId, params={}){
    ...
    if ( onboardingRecord.supervisor_id ) {
      ...
      if ( supervisor.iamRecord.employeeId != iamRecord.supervisorEmployeeId ) {
    ...
    if ( onboardingRecord.additional_data.isDeptHead ) {
      ...
      if ( currentHead.res.rows.length && !forceAdoption ) {
        out.error = true;
        out.message = `Department head already exists for department '${orgGroup.name}'. Remove the current department head before adopting this employee`;
        out.canForce = false;
        return out;
      }
    }
