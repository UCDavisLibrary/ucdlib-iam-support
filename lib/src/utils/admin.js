...
class iamAdmin {
  ...
  async adoptEmployee(onboardingId, params={}){
    ...
    // assign employee to groups
    const addedGroups = [];
    for (const group of groups) {
      let isHead = onboardingRecord.additional_data.isDeptHead && group.part_of_org;
      if ( isHead ) {
        let removeCurrentHead = await UcdlibGroups.removeGroupHead(group.id);
      ...
      let addGroupMembership = await UcdlibEmployees.addEmployeeToGroup(newEmployeeId, group.id, isHead);
      if ( addGroupMembership.err ) {
        ...
        await UcdlibEmployees.delete(newEmployeeId);
        out.error = true;
        out.message = `Error adding employee to group '${group.id}': ${addGroupMembership.err.message}`;
        out.canForce = false;
        return out;
      }
    }
  ...
  extractAndPopulateEmployeeFields(args) {
    ...
    const fields = [
      ...
      {employeeTable: 'title', value: title, raise: true},
      {
        ...
        value: supervisorId,
        raise: customSupervisor ? false : true,
    ...
    for (const field of fields) {
      ...
      if ( !field.value && field.raise && !force ) {
        return {error: {error: true, message: `Missing value for '${field.employeeTable}' in IAM record.${field.msg ? " " + field.msg : ""}`, canForce: true}};
      }
    }
  ...
  async checkOnboardingUcdIamRecord(idOrRecord, params={}) {
    ...
    // retrieve ucd iam record
    const obUtils = new OnboardingIsoUtils(onboardingRecord);
