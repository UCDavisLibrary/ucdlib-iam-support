...
class OnboardingStore extends BaseStore {
  ...
  constructor() {
    ...
    this.events = {
      ...
      ONBOARDING_ADOPTION: 'onboarding-adoption'
    };
  }
  ...
  adoptionLoading(request, id) {
    this._setAdoptionState({
      state : this.STATE.LOADING,
      request
    }, id);
  }

  adoptionLoaded(payload, id) {
    this._setAdoptionState({
      state : this.STATE.LOADED,
      payload
    }, id);
  ...
  _setAdoptionState(state, id) {
    this.data.adoption[id] = state;
    this.emit(this.events.ONBOARDING_ADOPTION, state);
  }
