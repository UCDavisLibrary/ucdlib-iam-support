...
class PersonStore extends BaseStore {
  ...
  _setPersonByIdState(state) {
    ...
    // save by IAM id too to save an extra call later
    if ( 
      state.state === this.STATE.LOADED && 
      this.searchParams[state.idType].hasDetailedData &&
      state.payload.iamId
    ) {
      this.data.iamId[state.payload.iamId] = state;
    }
