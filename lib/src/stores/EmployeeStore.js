...
class EmployeeStore extends BaseStore {
  ...
  constructor() {
    ...
    this.data = {
      ...
      activeDiscrepancies: new LruStore({name: 'employee.active-discrepancies'}),
      removeActiveDiscrepancies: {}
    };
    this.events = {
      ...
      GET_ACTIVE_DISCREPANCIES: 'get-active-discrepancies',
      REMOVE_ACTIVE_DISCREPANCIES: 'remove-active-discrepancies'
    };
  }
  ...
  getActiveDiscrepancyLoading(request, id) {
    this._getActiveDiscrepancy({
      state : this.STATE.LOADING,
      request, id
    });
  }

  getActiveDiscrepancyLoaded(payload, id) {
    this._getActiveDiscrepancy({
      state : this.STATE.LOADED,
      payload, id
    });
  ...
  getActiveDiscrepancyError(error, id) {
    this._getActiveDiscrepancy({
      state : this.STATE.ERROR,
      error, id
    });
  ...
  _getActiveDiscrepancy(state) {
    this.data.activeDiscrepancies.set(state.id, state);
    this.emit(this.events.GET_ACTIVE_DISCREPANCIES, state);
  }

  removeActiveDiscrepancyLoading(request, id, discrepanciesList) {
    this._removeActiveDiscrepancy({
      state : this.STATE.LOADING,
      request, id, discrepanciesList
    });
  }

  removeActiveDiscrepancyLoaded(payload, id, discrepanciesList) {
    this._removeActiveDiscrepancy({
      state : this.STATE.LOADED,
      payload, id, discrepanciesList
    });
  }

  removeActiveDiscrepancyError(error, id, discrepanciesList) {
    this._removeActiveDiscrepancy({
      state : this.STATE.ERROR,
      error, id, discrepanciesList
    });
  }

  _removeActiveDiscrepancy(state) {
    this.data.removeActiveDiscrepancies = state;
    this.emit(this.events.REMOVE_ACTIVE_DISCREPANCIES, state);
  }
