...
class SeparationStore extends BaseStore {
  ...
  constructor() {
    ...
    this.events = {
      ...
      SEPARATION_SUBMISSION_PUT: 'separation-submission-put',
      ...
      SEPARATION_DEPROVISION: 'separation-deprovision'
    };
  }
  ...
  deprovisionLoading(request, id) {
    this._setDeprovisionState({
      state : this.STATE.LOADING,
      request
    }, id);
  }

  deprovisionLoaded(payload, id) {
    this._setDeprovisionState({
      state : this.STATE.LOADED,
      payload
    }, id);
  ...
  _setDeprovisionState(state, id) {
    this.data.deprovision[id] = state;
    this.emit(this.events.SEPARATION_DEPROVISION, state);
  }

  byChangeIdLoading(request, id) {
    this._setChangeByIdState({
      state : this.STATE.LOADING,
      request
    }, id);
  }

  byChangeIdLoaded(payload, id) {
    this._setChangeByIdState({
      state : this.STATE.LOADED,
      payload
    }, id);
  ...
  _setChangeByIdState(state, id) {
    this.data.byId[id] = state;
    this.emit(this.events.SEPARATION_SUBMISSION_PUT, state);
  }
