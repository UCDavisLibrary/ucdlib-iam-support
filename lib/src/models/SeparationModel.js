...
class SeparationModel extends BaseModel {
  ...
  async reconcile(separationId, iamId) {
    let state = this.store.data.reconciliation[separationId];
    try {
      if( state && state.state === 'loading' ) {
        await state.request;
      } else {
        await this.service.reconcile(separationId, iamId);
      }
    } catch(e) {}
    return this.store.data.reconciliation[separationId];
  }

  async getById(id, change={}) {
    let state = this.store.data.byId[id];
    try {
      if( state && state.state === 'loading' ) {
        await state.request;
      } else {
        await this.service.getById(id, change);
      }
    } catch(e) {}
