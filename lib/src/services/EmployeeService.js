...
class EmployeeService extends BaseService {
  ...
  getAll(){
    return this.request({
      url : '/api/employees',
      onLoading : request => this.store.getAllLoading(request),
      checkCached : () => this.store.data.all,
      onLoad : result => this.store.getAllLoaded(result.body),
      onError : e => this.store.getAllError(e)
    });
  }
