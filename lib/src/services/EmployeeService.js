...
class EmployeeService extends BaseService {
  ...
  getActiveDiscrepancy(id){
    return this.request({
      ...
      checkCached : () => this.store.data.activeDiscrepancies,
      onLoad : result => this.store.getActiveDiscrepancyLoaded(result.body, id),
      onError : e => this.store.getActiveDiscrepancyError(e, id)
    });
  }

  removeActiveDiscrepancy(id, discepanciesList){
    return this.request({
      url : `/api/employees/${id}/discrepancies`,
      fetchOptions : {
        method : 'POST',
        body: discepanciesList
      },
      json: true,
      onLoading : request => this.store.removeActiveDiscrepancyLoading(request, id, discepanciesList),
      checkCached : () => this.store.data.removeActiveDiscrepancies,
      onLoad : result => this.store.removeActiveDiscrepancyLoaded(result.body, id, discepanciesList),
      onError : e => this.store.removeActiveDiscrepancyError(e, id, discepanciesList)
    });
  }
