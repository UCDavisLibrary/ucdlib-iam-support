...
import * as Templates from "./ucdlib-iam-separation-search.tpl.js";

/**
 * @description Component element for querying the UC Davis IAM Separation API
 */
export default class UcdlibIamSeparationSearch extends window.Mixin(LitElement)
  ...
  constructor() {
    ...
    this._injectModel('PersonModel', "SeparationModel");
    ...
    this.hideNav = false;
    this.hideNavOptions = '';
    this.widgetTitle = 'UC Davis Separation Search';
  ...
  async _onSubmit(e){
    ...
    let r = await this.SeparationModel.recordSearch(q);

    if ( r.state === this.SeparationModel.store.STATE.LOADED ) {
      this.isFetching = false;
      this.results = Array.isArray(r.payload) ? r.payload : [r.payload];
      if ( !this.hideResults ){
        this.page = 'results';
      }
    } else if( r.state === this.SeparationModel.store.STATE.ERROR ) {
      this.isFetching = false;
    ...
    this.dispatchEvent(new CustomEvent('separation-search', {detail: {status: r}}));

  }

  _onSelect(result){
    this.reset();
    this.dispatchEvent(new CustomEvent('separation-select', {detail: {record: result}}));
  }

}

customElements.define('ucdlib-iam-separation-search', UcdlibIamSeparationSearch);
