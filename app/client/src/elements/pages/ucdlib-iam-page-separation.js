...
import * as Templates from "./ucdlib-iam-page-separation.tpl.js";
 ...
 */export default class UcdlibIamPageSeparation extends window.Mixin(LitElement)
 ...
 static get properties() {
  ...
  openEmployeeInfoModal(){
    const ele = this.renderRoot.querySelector('#sep-employee-modal');
    if ( ele ) ele.show();
  }
  ...
  async _onEmployeeSelect(response){
    if( response.state === this.PersonModel.store.STATE.LOADED ) {
      ...
      console.log("IAM:",this.iamRecord);
   ...
   */
  _resetLookupForms(){
    this.renderRoot.querySelector('#sep-lookup ucdlib-iam-search' ).reset();
    this.renderRoot.querySelector('#sep-manual ucdlib-iam-search' ).reset();
   ...
   */
  _onSupervisorEdit(){
    const modal = this.renderRoot.querySelector('#sep-custom-supervisor');
    if ( modal ) modal.show();
  }
