...
export function render() {
  ...
  <div class='l-container u-space-pb'>
    ...
    <ucdlib-iam-modal id='sep-employee-modal' dismiss-text='Close' content-title='Employee Record'>
      ${!this.userEnteredData ? html`<pre style='font-size:15px;margin:0;'>${JSON.stringify(this.iamRecord.data, null, "  ")}</pre>` : html``}
    </ucdlib-iam-modal>
    <ucdlib-iam-modal id='sep-custom-supervisor' dismiss-text='Close' content-title='Set a Custom Supervisor' auto-width>
    ...
    <form class='form-single-col' @submit=${this._onSubmit}>
        ...
        <div class="panel panel--icon panel--icon-custom o-box panel--icon-double-decker">
          ...
          <section>
            ...
            <div class="field-container">
              <label for="sep-appointments">Appointment</label>
              <select id="sep-appointments" @input=${(e) => this._onAppointmentSelect(e.target.value)}>
      ...
      <div>
        <div class="panel panel--icon panel--icon-custom o-box panel--icon-quad">
          ...
          <section>
            <div class="field-container">
              <label for="sep-title">Position Title <abbr title="Required">*</abbr></label>
              <input id='sep-title' type="text" .value=${this.positionTitle} required @input=${e => this.positionTitle = e.target.value}>
            </div>
            <div class="field-container">
              <label for="sep-departments">Department <abbr title="Required">*</abbr></label>
              <select id="sep-departments" required @input=${(e) => this.departmentId = e.target.value} .value=${this.departmentId || ''}>
            ...
            <div class="checkbox">
              <ul class="list--reset">
                <li>
                  <input id="sep-is-dept-head" type="checkbox" @input=${() => this.isDeptHead = !this.isDeptHead} .checked=${this.isDeptHead}>
                  <label for="sep-is-dept-head">Is Department Head</label>
                </li>
              </ul>
            </div>
            <div class="field-container">
            ...
            <div class="field-container">
              <label for="sep-start-date">Start Date <abbr title="Required">*</abbr></label>
              <input id='sep-start-date' type="date" required .value=${this.startDate} @input=${(e) => {this.startDate = e.target.value;}}>
            </div>
          </section>
        </div>
      </div>
      ...
      <div class="panel panel--icon panel--icon-custom o-box panel--icon-delta">
        ...
        <section>
          <div class="field-container">
            ...
            <input id='sep-supervisor' type="text" .value=${this.supervisor.fullName} disabled >
          </div>
          <div class="checkbox">
            <ul class="list--reset">
              <li>
                <input id="sep-skip-supervisor" type="checkbox" @input=${() => this.skipSupervisor = !this.skipSupervisor} .checked=${this.skipSupervisor}>
                <label for="sep-skip-supervisor">Do not notify supervisor</label>
              </li>
            </ul>
          </div>
          <div class="field-container" ?hidden=${this.skipSupervisor}>
            <label for="sep-supervisor-email">Supervisor Email for RT Ticket</label>
            <input id='sep-supervisor-email' type="text" @input=${e => this.supervisorEmail = e.target.value} .value=${this.supervisorEmail} >
          </div>
        </section>
      </div>
      <div class="panel panel--icon panel--icon-custom o-box panel--icon-poppy">
        ...
        <section>
          <div class="field-container">
            <label for="sep-notes">Notes</label>
            <textarea id='sep-notes' rows="8" cols="48"  @input=${e => this.notes = e.target.value} .value=${this.notes}></textarea>
          </div>
        </section>
      </div>
      <button
...
export function renderEmployeeForm(){
  const isSub = this.page == 'sep-submission';
  const disabled = (!this.userEnteredData && isSub);
  return html`
    <div class="field-container">
      <label for="sep-first-name">First Name</label>
      <input id='sep-first-name' type="text" .value=${this.firstName} ?disabled=${disabled} @input=${e => this.firstName = e.target.value}>
    </div>
    <div class="field-container">
      <label for="sep-last-name">Last Name</label>
      <input id='sep-last-name' type="text" .value=${this.lastName} ?disabled=${disabled} @input=${e => this.lastName = e.target.value}>
    </div>
    ...
    <div class="field-container">
      <label for="sep-employee-id">Employee Id</label>
      <input id='sep-employee-id' type="text" .value=${this.employeeId} ?disabled=${disabled} @input=${e => this.employeeId = e.target.value}>
    </div>
    <div class="field-container">
      <label for="sep-user-id">Kerberos</label>
      <input id='sep-user-id' type="text" .value=${this.userId} ?disabled=${disabled} @input=${e => this.userId = e.target.value}>
    </div>
    <div class="field-container">
      <label for="sep-email">UC Davis Email</label>
      <input id='sep-email' type="text" .value=${this.email} @input=${e => this.email = e.target.value}>
    </div>
  `;
}
